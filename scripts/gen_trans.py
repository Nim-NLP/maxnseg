...
model_dir = path.join(cur_dir,"..","model")
word_count_path = path.join(model_dir,"word_dict.model")
word_trans_path = path.join(model_dir,"trans_dict.model")
    ...
    for pre_word, post_info in Trans_dict.items():
        for post_word, count in post_info.items():
            ...
            if pre_word in word_dict_count.keys():
                trans_dict[key] = math.log(count / word_dict_count[pre_word])  # 取自然对数，归一化
            else:
...
if __name__ == "__main__":
    ...
    result += TEMPLATE % ("wordProb", re.sub(r'"([BMES])"',r"'\1'",json.dumps(word_dict,ensure_ascii=False, indent=2))
                    .replace("}", "}.newTable")
                    )
    result += TEMPLATE % ("wordFreq", re.sub(r'"([BMES])"',r"'\1'",json.dumps(word_dict_count,ensure_ascii=False, indent=2))
                    .replace("}", "}.newTable")
                    )
    result += TEMPLATE % ("transFreq", re.sub(r'"([BMES])"',r"'\1'",json.dumps(trans_dict_count,ensure_ascii=False, indent=2))
                    .replace("}", "}.newTable")
                    )
